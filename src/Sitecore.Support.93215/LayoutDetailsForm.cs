// Sitecore.Shell.Applications.ContentManager.Dialogs.LayoutDetails.LayoutDetailsForm
using Sitecore;
using Sitecore.Data;
  ...
  public class LayoutDetailsForm : DialogForm
  {
    ...
    protected void CopyDevicePipeline(ClientPipelineArgs args)
    {
      ...
      if (args.IsPostBack)
      {
        if (!string.IsNullOrEmpty(args.Result) && args.Result != "undefined")
        {
          ...
          string @string = StringUtil.GetString(args.Parameters["deviceid"]);
          ListString devices = new ListString(array[0]);
          string text = array[1];
          XmlNode xmlNode = this.GetDoc().SelectSingleNode("/r/d[@id='" + @string + "']");
      ...
      else
      {
        ...
        urlString["de"] = StringUtil.GetString(args.Parameters["deviceid"]);
    ...
    protected override void OnLoad(EventArgs e)
    {
      ...
      if (!Context.ClientPage.IsEvent)
      {
        ...
        {
          this.LayoutDelta = (field.GetValue(false, false) ?? field.GetInheritedValue(false) ?? field.GetValue(false, false, true, false, false));
        }
        else
        {
    ...
    private static void CopyDevices(XmlDocument doc, ListString devices, XmlNode sourceDevice)
    {
      ...
      foreach (string device in devices)
      {
        ...
        {
          XmlNode xmlNode2 = doc.DocumentElement.SelectSingleNode("d[@id='" + device + "']");
          if (xmlNode2 != null)
          {
    ...
    private static string GetLayoutValue(XmlDocument doc)
    {
      ...
      if (xmlNodeList != null && xmlNodeList.Count != 0)
      {
        ...
        {
          if (item.ChildNodes.Count > 0 || XmlUtil.GetAttribute("l", item).Length > 0)
          {
            return doc.OuterXml;
          }
        }
    ...
    private void RenderLayoutGridBuilder(string layoutValue, Control renderingContainer)
    {
      ...
      LayoutGridBuilder obj = new LayoutGridBuilder
      {
        ID = iD,
        Value = layoutValue,
      ...
      obj.BuildGrid(renderingContainer);
      if (Context.ClientPage.IsEvent)
      {
